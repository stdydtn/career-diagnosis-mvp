...
export function getReadinessLevel(maturityAvg, readinessAvg) {
  ...
  if (readinessAvg < 3.3) {
    return {
      ...
      bullets: [
        ...
        "경험을 '어떤 상황이었는지 → 내가 무엇을 했는지 → 결과가 어땠는지' 순서로 3개만 정리해 두면 자소서 문항 대부분에 활용할 수 있어요.",
  ...
  return {
    ...
    bullets: [
      ...
      "모의 면접이 부담되면 핵심 경험 3개를 친구에게 말로만 설명해 보고, 빠진 숫자·결과를 채워 넣으세요.",
      "지원 결과가 기대와 달랐을 때는 서류·면접 중 어느 부분이 아쉬웠는지 한 줄씩만 기록해 두면 다음 지원에 바로 반영할 수 있어요.",
    ],
  };
}
...
export function createDetailedReport(result, profile, feedback) {
  ...
  return {
    ...
    actionPlan: [
      ...
      "경험은 '상황 → 내가 한 일 → 결과' 형식으로 3개만 정리해 두세요. 숫자가 없으면 기간·참여 인원·완료 여부처럼 적어도 되는 단위부터 채워 넣어 보세요.",
      ...
      "결과와 상관없이 지원한 공고 링크와 제출한 파일 날짜를 한 줄씩만 기록해 두세요. 다음 수정 포인트를 찾기 쉬워집니다.",
    ],
